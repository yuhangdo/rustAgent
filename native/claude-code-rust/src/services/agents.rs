...
Be thorough and systematic. Focus on finding and reporting issues."#.to_string(),
    ...
    async fn execute_agent(&self, agent: &AgentDefinition, prompt: &str) -> anyhow::Result<String> {
        ...
        match outcome {
            crate::agent_runtime::AgentExecutionOutcome::Completed(result) => Ok(result.answer),
            crate::agent_runtime::AgentExecutionOutcome::Cancelled => {
                Ok("Agent run cancelled.".to_string())
            }
        }
    }
